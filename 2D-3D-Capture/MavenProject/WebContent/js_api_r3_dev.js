		...
		setValues : function(){
			...
			if(this.Type =="Mobile"){
				try{
					...
					alpha = temp.alpha; beta = temp.abeta; gamma = temp.gamma;
				} catch(error){
...
//									document.getElementById("current_reading").innerHTML=interval;
...
//			metadata =JSON.stringify({'vwidth': 640, 'browsertype': 'Chrome', 'ext': 'png', 'devicetype': 'Android', 'vheight': 480, 'time': '2013.11.28_9.15.15', 'device': {'orientation': 'potrait', 'gz': 6.74, 'gy': 7.18, 'gx': 0, 'rg': 6.7995, 'ra': 322.6541, 'rb': 46.9973, 'ay': 0.13, 'ax': 0, 'az': 0}, 'position': {'lat': 65.01242298532382, 'acc': 128, 'alt': 142.17365173309813, 'lon': 25.47399834095566}, 'type': 'image'});
		...
		upladlImageWithWebSocket : function() {
			...
			var imgPostRequest = $.post( "http://"+this.serverURL +":"+this.serverPORT+"/postImage",{ },function(response) {
				response = response.trim();
				if(response == "READY"){
					try {
						var message =JSON.stringify(this.imagedata);
						var r = confirm("CONFIRM :" +response+"-->"+message) ;
						if(r){
							...
							this.connection.onmessage =	function (event) {					
								...
								console.log("MESSAGE RECIEVED " +svrmessage.trim());
								if(svrmessage.trim() =="READY"){
		...
		upladlImageWithPost : function() {
			...
			if(r){
				...
				//alert("http://"+this.serverURL+":"+this.serverPORT+"/postBinaryImage");
...
//				xhr.setRequestHeader("Content-type","binary/octet-stream");
				...
				xhr.open('POST', "http://"+this.serverURL+":"+this.serverPORT+"/postBinaryImage", true);
				...
				xhr.onload = function(e) {
					if(e.currentTarget.readyState == 4  ) {
						if(e.currentTarget.status == 200){
							...
							document.body.removeChild(localcanvas);
							var icon;
							icon =document.getElementById("Posting");
							if(icon)
								document.body.removeChild(icon);
							alert("SUCCESS");
