...
class EventManager:
    ...
    def unsubscribe(self, sub_id):
        ...
        try:
            ...
            conn.send(self.encodeMessage("{ \"type\" : 2 }"))
            conn.close()
        except KeyError :
            pass
    ...
    #             message ="{ \"type\" : 3, data : { \"url\" : \"%s/%s\" , \"alpha\" : %s , \"beta\" :%s , \"gamma\" :%s, \"lat\" :%s , \"long\" :%s }}"%(repo_url,filename,jsondata["device"]["ra"],jsondata["device"]["rb"],jsondata["device"]["rg"],["position"]["lat"],jsondata["position"]["lon"])
                message ="{ \"type\" : 3, \"url\" : \"%s/%s\", \"alpha\" : %s , \"beta\" :%s , \"gamma\" :%s, \"lat\" :%s , \"long\" :%s, \"height\" : %s , \"width\": %s} "%(repo_url, filename, str(jsondata["device"]["ra"]), str(jsondata["device"]["rb"]), str(jsondata["device"]["rg"]),str(jsondata["position"]["lat"]), str(jsondata["position"]["lon"]),str(jsondata["vwidth"]),str(jsondata["vheight"]))
