(function() {
    ...
    function getElements(layerName, lowerCornerX, lowerCornerY, higherCornerX, higherCornerY, layerCRS, transformX, transformY){
        ...
        if (layerName.indexOf("building_coordinates")>=0){
            ...
            var xml3dAssetUrl = createGISAssetUrl( baseUrl, 
                                        layerName, 
                                        lowerCornerX+","+
                                        lowerCornerY+","+
                                        higherCornerX+","+
                                        higherCornerY,
                                        layerCRS);
            addXml3DContent(xml3dAssetUrl, layerName, texture, transformX, transformY);
        }
    ...
    function createGISAssetUrl(baseUrl, layer, boundingbox, layerCRS) {
        // console.log("createGISRequest");
        var requestUrl;
        var service = "w3ds?version=0.4&service=w3ds";        
        var format = "&format=Application/xml"; //Asset instance request type
        var crs = "&crs="+layerCRS;
        var request = "&request=GetScene";
        var assetAnchor = "#asset"; //Returned terrain contains id "asset"

        // If user hasn't defined LOD level, LOD level is not included to request at all
        if (LodLevel !== -1){
            requestUrl = baseUrl + service + request + crs + format+"&layers="+layer+"&boundingbox="+boundingbox+"&LOD="+LodLevel+assetAnchor;
        }else{
            requestUrl = baseUrl + service + request + crs + format+"&layers="+layer+"&boundingbox="+boundingbox+assetAnchor;
        }
    ...
    function addXml3DContent(xml3dAssetUrl, layerName, textureUrl, transformX, transformY) {
